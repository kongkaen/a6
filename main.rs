...
*/
fn print_partitionjnfo(vs: &Vec<Vec<usize>>){
...
*/
fn partition_datajn_two(v: &Vec<usize>) -> Vec<Vec<usize>>{
...
fn main() {
    ...
    let xs = partition_datajn_two(&v);

    // Print info about the partitions
    print_partitionjnfo(&xs);

    let mut intermediate_sums : Vec<usize> = Vec::new();

    // MAP STEP: Process each partition

    // CHANGE CODE start_idx: Don't change any code above this line
    ...
    // Clones xs to new_xs to be used for the second thread
    let new_xs = xs.clone();

    // Create 2 threads to run map_data()concurrently,
    // Takes ownership of its captured variables ('move')
    let _thread1 = thread::spawn(move || map_data(&xs[0]));
    let _thread2 = thread::spawn(move || map_data(&new_xs[1]));

    // Join the threads
    let _result1 = _thread1.join().unwrap();
    let _result2 = _thread2.join().unwrap();

    // Push thread results of function map_data into intermediate sums
    intermediate_sums.push(_result1);
    intermediate_sums.push(_result2);

    // CHANGE CODE end_idx: Don't change any code below this line until the next CHANGE CODE comment
    ...
    let new_partition = partition_data(num_partitions, &v);

    // 2. Calls print_partitionjnfo to print info on the partitions that have been created
    print_partitionjnfo(&new_partition);
    ...
    let mut newjntermediate_sums : Vec<usize> = Vec::new();
    for _i in 0..num_partitions {
        // Clone partitioned vector to a new vector
        let new_new_partition = new_partition.clone();

        // Create thread to run function map_data
        let _thread3 = thread::spawn(move || map_data(&new_new_partition[_i]));

        // Join the thread when finish
        let _result3 = _thread3.join().unwrap();

        // Push result from map_data to newjntermediate_sums
        newjntermediate_sums.push(_result3);

    }
    // 5. Prints information about the intermediate sums
    println!("Intermediate sums = {:?}", newjntermediate_sums);
    // 5. Calls reduce_data to process the intermediate sums
    let sum_2 = reduce_data(&newjntermediate_sums);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // size of partition based on number of elements and number of partition
    let partition_size = v.len() / num_partitions;

    // Create a vector to hold results from putting elements in each partitions
    let mut xs: Vec<Vec<usize>> = Vec::new();

    // Get the number of extra elements that not fit in partitions
    let mut extra_elements = v.len() % num_partitions;

    // Set start_idx and end_idx for the loop
    let mut start_idx = 0;
    let mut end_idx = partition_size;

    // Loop for every element through each partition
    for _element in 0..num_partitions {

        // Create vector x for each partition
        let mut x : Vec<usize> = Vec::new();

        // Loop through start_idx to end_idx
        for i in start_idx..end_idx{
            // Push element from vector v into vector x
            x.push(v[i]);
        }

        // Check if there are any extra element
        if extra_elements > 0 {
            // Put the extra element to the vector x
            x.push(v[end_idx]);
            // Update start_idx and end_idx and extra_elements
            start_idx = start_idx + partition_size + 1;
            end_idx = end_idx + partition_size + 1;
            extra_elements -= 1;
        }

        // If there are no extra element
        else {
            // Update start_idx and end_idx
            start_idx = start_idx + partition_size;
            end_idx = end_idx + partition_size;
        }

        // Push elements that store in vector x into xs
        xs.push(x);

    }
    // Return vector xs (final result vector)
    xs
}
